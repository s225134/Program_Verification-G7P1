        ...
        CmdKind::Loop { invariants, variant: _, body } => {
            ...
            for (i,case) in enumerate(&body.cases) {
                ...
                let else_branch = IVLCmd {
                    span: bi.span,
                    kind: IVLCmdKind::Seq(
                        Box::new(IVLCmd { span: bi.span, kind: IVLCmdKind::Assume { condition: Expr::not(bi) } }),
                        Box::new(IVLCmd { span: cmd.span, kind: IVLCmdKind::Assert { condition: Expr::bool(true), message: "Second branch".to_string() } }),
                    ),
                };
